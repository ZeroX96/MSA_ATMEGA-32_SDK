	...
	if (given_obj != NULL)
	{
		...
		*(volatile ecore_u8*)(given_obj->timer_base)|= given_obj->timer_freq;
		...
		if (given_obj->timer_base == TIMER_0)
		{
			...
			{	
				if (given_obj->int_mode == INTERRUPTABLE_AT_OV)
				{
					TIMSK|=(1<<0);	//enable interrupt at overflow
				} 
				else if (given_obj->int_mode == INTERRUPTABLE_AT_OC)
				{
					TIMSK|=(1<<1);	//enable interrupt at outputcompare
				}
				else if (given_obj->int_mode == INTERRUPTABLE_AT_OV_AND_OC)
				{
					TIMSK|=(3<<0);	//enable interrupt at overflow and oc
				}
				else
				{	//NON_INTERRUPTABLE
					TIMSK&=~(0x03);
		...
		else if (given_obj->timer_base == TIMER_2)
		{
			...
			if (given_obj->t_mode == NORMAL_MODE)
			{
				if (given_obj->int_mode == INTERRUPTABLE_AT_OV)
				{
					TIMSK|=(1<<TOIE2);	//enable interrupt at overflow
				}
				else if (given_obj->int_mode == INTERRUPTABLE_AT_OC)
				{
					TIMSK|=(1<<OCIE2);	//enable interrupt at outputcompare
				}
				else if (given_obj->int_mode == INTERRUPTABLE_AT_OV_AND_OC)
				{
					TIMSK|=(3<<TOIE2);	//enable interrupt at overflow and oc
				}
				else
				{	//NON_INTERRUPTABLE
					TIMSK&=~(0x03<<TOIE2);
			...
			else
			{
				...
				{
					TIMSK|=(1<<TOIE2);	//enable interrupt at outputcompare
				}
				else if (given_obj->int_mode == INTERRUPTABLE_AT_OV_AND_OC)
				{
					ret_val=INVALID_PARAMS;
		...
		//given_obj->timer_comp_val;
		//test later
		/*if CTC_MODE
		TCCR0 |= 0x40;
		
		if (given_obj->t_mode == CTC_MODE)
		{
			TCCR0|(1<<WGM01);
